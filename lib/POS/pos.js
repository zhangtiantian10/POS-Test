...
POS.prototype.getSubTotal = function (itemInfoCounts) {
  ...
  var itemsGetSubToal = itemInfoCounts.map((itemInfoCount) => {
    if (this.searchPromotions(itemInfoCount.item.barcode) === 1) {
      subTotal = itemInfoCount.item.price * (itemInfoCount.count - parseInt(itemInfoCount.count/3));
      ...
      subTotal = itemInfoCount.item.price * itemInfoCount.count * 95 / 100;
      saveTotal = itemInfoCount.item.price * itemInfoCount.count * 5 / 100;
    } else {
      subTotal = itemInfoCount.item.price * itemInfoCount.count;
      saveTotal = 0;
    }
...
POS.prototype.print = function (itemsGetTotal) {
  ...
  itemsGetTotal.itemsGetSubToal.forEach((itemGetSubToal) => {
    ...
    printText += '名称：' + item.name + '，数量：' + itemGetSubToal.itemInfoCount.count +
    item.unit + '，单价：' + parseFloat(item.price).toFixed(2) +
    '(元)，小计：' + parseFloat(itemGetSubToal.subTotal).toFixed(2) + '(元)'
    if(this.searchPromotions(item.barcode) === 2) {
      printText += '，节省' + parseFloat(itemGetSubToal.saveTotal).toFixed(2) + '(元)\n'
    } else {
      printText += '\n';
    }
  ...
  if(itemsGetTotal.saved != 0) {
    printText += '节省：' + parseFloat(itemsGetTotal.saved).toFixed(2) + '(元)\n'
  }
  printText += '**********************';
